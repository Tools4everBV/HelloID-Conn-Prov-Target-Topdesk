#####################################################
# HelloID-Conn-Prov-Target-TOPdesk-Permissions-Groups-Grant
# PowerShell V2
#####################################################
...
function New-TopdeskBodyAdd {
    param(
        [string]$PersonField,  # veldnaam
        [string]$Value,        # nieuwe waare
        [string]$FieldValue    # bestaande waarde
    )

    $parts = $PersonField -split '\.'

    # Nieuwe waarde opbouwen voor het veld
    ...
    if (-Not([string]::IsNullOrEmpty($TopdeskPerson))) {
        ...
        switch ($action) {
            'SetGroupname' {
                ...
                $body = New-TopdeskBodyAdd -PersonField $PersonField -Value $GroupName -FieldValue $fieldValue
                ...
                if (-Not($actionContext.DryRun -eq $true)) {
                    ...
                    Write-Information "Account with id [$($TopdeskPerson.id)] successfully updated. $PersonField appended with $GroupName."

                    $outputContext.AuditLogs.Add([PSCustomObject]@{
                            Message = "Account with id [$($TopdeskPerson.id)] successfully updated. $PersonField appended with $GroupName."
                            IsError = $false
                        })
                }
                else {
                    Write-Warning "DryRun would append $PersonField with $GroupName."
                }
            ...
            'NoChanges' {        
                Write-Information "Account with id [$($TopdeskPerson.id)] not updated. $PersonField already contains $GroupName. No changes required."
                break
            }
