...
$change = $entitlementSet.Revoke

if ([string]::IsNullOrEmpty($entitlementSet)) {
    write-verbose -Verbose -Message ($entitlementSet | ConvertTo-Json)
    throw "could not find entitlement set"
    $auditMessage = "$($entitlementSet.displayname) niet gevonden"
}
...
if (![string]::IsNullOrEmpty($change)) {
   if (-Not($dryRun -eq $true)) {
    ...
    #Change is empty
    $auditMessage = "$($entitlementSet.displayname) not created, Revoke is not configured."
    if (-Not($dryRun -eq $true)) {
        $success = $true
    }
}
